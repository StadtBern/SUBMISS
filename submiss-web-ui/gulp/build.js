...
gulp.task('main-bower-images', function() {
  return gulp.src($.mainBowerFiles({filter: ['**/*.gif', '**/*.jpg', '**/*.png']}))
  .pipe(gulp.dest(path.join(conf.paths.dist, '/images/')));
});

gulp.task('customFonts', function () {
  return gulp.src([
    path.join(conf.paths.src, '/assets/css/fonts/**/*')
  ])
  .pipe(gulp.dest(path.join(conf.paths.dist, '/fonts/')));
});
...
gulp.task('build', ['html', 'fonts', 'customFonts', 'main-bower-images', 'other']);
