...
public class CompanyProofsValidityCheckScheduler implements SchedulerJob {
  ...
  public void execute(Map<String, Object> dataMap) {
    ...
    for (CompanyEntity company : companies) {
      ...
      if (setActive) {
        company.setProofStatus(ProofStatus.ALL_PROOF.getValue());
        // update also the proof status Fabe if it is set to automatic status
        if (company.getProofStatusFabe().equals(ProofStatus.NOT_ACTIVE.getValue())) {
          company.setProofStatusFabe(ProofStatus.ALL_PROOF.getValue());
        }
        em.merge(company);
      } else {
