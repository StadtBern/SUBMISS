...
import ch.bern.submiss.services.api.administration.SDNegotiatedProcedureService;
...
/**
 * The Class SDNegotiatedProcedureServiceImpl.
 */
@Transactional
@OsgiServiceProvider(classes = {SDNegotiatedProcedureService.class})
@Singleton
public class SDNegotiatedProcedureServiceImpl extends BaseService
  implements SDNegotiatedProcedureService {

  /**
   * The Constant LOGGER.
   */
  private static final Logger LOGGER =
    Logger.getLogger(SDNegotiatedProcedureServiceImpl.class.getName());

  /*
   * (non-Javadoc)
   *
   * @see ch.bern.submiss.services.api.administration.SDNegotiatedProcedureService#readAll()
   */
  @Override
      ...
      query.select(qMasterListValueHistoryEntity).from(qMasterListValueHistoryEntity)
        .where(
          qMasterListValueHistoryEntity.masterListValueId.masterList.categoryType
            .equalsIgnoreCase(CategorySD.NEGOTIATION_REASON.getValue()),
